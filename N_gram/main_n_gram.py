...
# API endpoint to search using query parameter
@app.get("/search/", response_model=MatchResult)
async def search_record(query: str = Query(..., description="String to search for"), threshold: float = 40.0):
    n = 2  # Using bigrams for n-grams
    all_records = await collection.find().to_list(length=None)
    best_match = None
    best_similarity = 0
    
    for record in all_records:
        record_similarity = calculate_similarity(query, record['name'], n)
        
        # Debugging: Print the similarity score for each record
        print(f"Comparing '{query}' with '{record['name']}': Similarity = {record_similarity:.2f}%")
